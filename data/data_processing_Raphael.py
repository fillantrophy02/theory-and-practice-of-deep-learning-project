...
class DataProcessingPipeline():
    ...
    def clean(self):
        ...
        self._fill_missing_rain_today_from_previous_rain_tomorrow()
    ...
    def _fill_missing_rain_today_from_previous_rain_tomorrow(self):
        if 'RainToday' in self.df.columns and 'RainTomorrow' in self.df.columns:
            def fill_group(group):
                group = group.sort_values('Date').copy()
                mask = group['RainToday'].isna()
                group.loc[mask, 'RainToday'] = group['RainTomorrow'].shift(1)[mask]
                return group

            self.df = self.df.groupby('Location').apply(fill_group).reset_index(drop=True)
    ...
    def _transform_categorical_features_to_numerical(self):
        ...
        for col in cat_columns:
            if col in self.df.columns:
                self.df[col] = self.df[col].map(lambda x: mapping.get(x.strip(), x) if isinstance(x, str) else x)

        # ensure consistent float type (important for modeling)
        if 'RainToday' in self.df.columns:
            self.df['RainToday'] = self.df['RainToday'].astype(float)
        if 'RainTomorrow' in self.df.columns:
            self.df['RainTomorrow'] = self.df['RainTomorrow'].astype(float)
    ...
    def prepare_tensor_data_classifier(self, sequence_length=1):
        ...
        X_raw = self.df[FEATURES].values.astype(np.float32)
        Y_raw = self.df[TARGET].values.astype(np.float32)

        if sequence_length > 1:
            X_seq, Y_seq = [], []
            for i in range(len(X_raw) - sequence_length):
                X_seq.append(X_raw[i:i+sequence_length])
                Y_seq.append(Y_raw[i+sequence_length - 1])  # predict end of window
            X_tensor = torch.tensor(np.stack(X_seq))  # shape: (samples, seq_len, features)
            Y_tensor = torch.tensor(Y_seq, dtype=torch.float32)            # shape: (samples,)
        else:
            X_tensor = torch.tensor(X_raw, dtype=torch.float32)  # shape: (samples, features)
            Y_tensor = torch.tensor(Y_raw, dtype=torch.float32)

        return X_tensor, Y_tensor
    
    def prepare_tensor_data_seq2seq(self, sequence_length=10, future_steps=5):
        FEATURES = [
        'MinTemp', 'MaxTemp', 'Rainfall',
        'WindGustSpeed', 'WindSpeed9am', 'WindSpeed3pm',
        'Humidity9am', 'Humidity3pm',
        ...
        grouped = self.df.groupby('Location')  # Group by location
        for _, group in grouped:
            group = group.sort_values('Date')  # Sort by date
            X_raw = group[FEATURES].values.astype(np.float32)
            Y_raw = group[TARGET].values.astype(np.float32)

            total_length = sequence_length + future_steps - 1
            for i in range(len(group) - total_length):
                X_seq.append(X_raw[i:i+sequence_length])
                Y_seq.append(Y_raw[i+sequence_length:i+sequence_length+future_steps])

        X_tensor = torch.tensor(np.stack(X_seq))  # (samples, seq_len, features)
        Y_tensor = torch.tensor(np.stack(Y_seq))  # (samples, future_steps)
